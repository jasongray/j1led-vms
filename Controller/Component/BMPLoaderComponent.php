...
class BMPLoaderComponent extends Component  {
			...
			case 32:
				for ($i = $this->data['BITMAP_HEADER']['Height'] - 1; $i >= 0; $i--) {
					for ($j = 0; $j < $this->data['BITMAP_HEADER']['Width']; $j++) {
						...
						$r = ord($data[$p + 2]);
						$a = ord($data[$p + 3]);
						$color = imagecolorallocatealpha($this->data['IMAGE'], $r, $g, $b, $a / 255 * 127);
						imagesetpixel($this->data['IMAGE'], $j, $i, $color);
						$this->data['matrix'][$i][$j] = "$r,$g,$b";
						$p += 4;
					}
				}
				break;
	...
	public function getMatrix() {
		return array_reverse($this->data['matrix']);
	}
