...
/** Google Maps JS **/
	...
	public function map($options = array()) {
		...
		$map = "
			...
			// deprecated
			gMarkers".self::$MAP_COUNT." = [];
			gInfoWindows".self::$MAP_COUNT." = [];
			gWindowContents".self::$MAP_COUNT." = [];
		";
		...
		if (!isset($this->_currentOptions['div']['inline']) || $this->_currentOptions['div']['inline'] != false) {
	...
	public function addMarker($options) {
		...
		*/
		...
		if (!empty($options['ajax'])) {
			...
			$_str = (!empty($this->_currentOptions['marker']['infoWindow']))? "[".$this->_currentOptions['marker']['infoWindow']."]": '';
			
			//$event = "load_content(".$this->name().", gInfoWindows".self::$MAP_COUNT.$_str.", gMarkers".self::$MAP_COUNT."[".self::$MARKER_COUNT."], '".$options['id']."');";
			$event = "
			gInfoWindows".self::$MAP_COUNT."[".$this->_currentOptions['marker']['infoWindow']."].setContent($('.accordion-body[data-sign=\"".$options['id']."\"]').html());
			gInfoWindows".self::$MAP_COUNT."[".$this->_currentOptions['marker']['infoWindow']."].open(".$this->name().", gMarkers".self::$MAP_COUNT."[".self::$MARKER_COUNT."]);
			myframeshow($('.accordion-inner[data-inner=\"".$options['id']."\"] .frameshow'));
			";
