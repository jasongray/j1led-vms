...
class VideoHelper extends HtmlHelper {
    ...
    public function embed($url, $settings = array()) {
        ...
        } elseif ($this->getVideoSource($url) == 'local') {
            return $this->localEmbed($url, $settings);
    ...
    public function localEmbed($url, $settings = array()) {
    	
    	return '<div class="flowplayer" data-volume="0.4">
				<video advance loop>
					<source type="video/mp4" src="/app/webroot' . $url . '" />
				</video>
			</div>';
    }
